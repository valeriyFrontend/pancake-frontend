...
export class Pool {
  ...
  public constructor({
    ...
    ;[this.token0, this.token1] = sortCurrencies([tokenA, tokenB])
    this.fee = fee
  ...
  public involvesToken(token: Token): boolean {
    if (token.isNative && this.token0.isNative) return true
  ...
  private async swap(
    ...
    const sqrtPriceLimitX96 =
      _sqrtPriceLimitX96 ?? (zeroForOne ? TickMath.MIN_SQRT_RATIO + ONE : TickMath.MAX_SQRT_RATIO - ONE)
    if (zeroForOne) {
