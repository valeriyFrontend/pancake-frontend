 ...
 */
export const getPriceX128FromId = (activeId: bigint, binStep: bigint): bigint => {
...
export const getPriceFromId = (activeId: number, binStep: number): number => {
  return (1 + binStep / 10_000) ** Number(activeId - 8388608)
}

export const getCurrencyPriceFromId = (
  activeId: number,
  binStep: number,
  baseCurrency: Currency,
  quoteCurrency: Currency
): Price<Currency, Currency> => {
  const price = new BigNumber(getPriceFromId(activeId, binStep))
...
export const getInvertedPriceFromId = (activeId: number, binStep: number): number => {
  return (1 + binStep / 10_000) ** Number(8388608 - activeId)
}
