...
export function VisualSlider(props: {
  ...
  return (
    ...
    >
      {/* to hold prev node  */}
      <Box ref={prevBoxRef} position="absolute" inset={0} style={{ transform: 'translateX(-100%)', transition: 'inherit' }} />
      ...
      {/* to hold next node  */}
      <Box ref={nextBoxRef} position="absolute" inset={0} style={{ transform: 'translateX(100%)', transition: 'inherit' }} />
    </Box>
  )
}
