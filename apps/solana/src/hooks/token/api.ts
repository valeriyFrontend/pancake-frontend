...
export function getTransferFeeConfig(mint: Mint): TransferFeeConfig | null {
  ...
  if (extensionData !== null) {
    return TransferFeeConfigLayout.decode(extensionData as any)
  }
  return null
}
    ...
    if (connection) {
      ...
      if (space === 82 || space >= 165) {
        ...
        if (info?.owner.equals(TOKEN_2022_PROGRAM_ID)) {
          pId = TOKEN_2022_PROGRAM_ID
          tags.push('token-2022')
        }
        const onlineData = unpackMint(address, info, pId)
        const mintAddress = mint.toString()

        try {
          if (info && MintLayout.decode(info?.data as any).freezeAuthorityOption === 1) tags.push('hasFreeze')
        } catch {
