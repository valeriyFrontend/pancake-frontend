...
const fetcher = async ([connection, id]: [Connection, string]) => {
  try {
    ...
    return {
      ...poolInfo,
      baseReserve: new BN(AccountLayout.decode(poolVaultAState.data as any).amount.toString())
        .sub(poolInfo.protocolFeesMintA)
        .sub(poolInfo.fundFeesMintA),
      quoteReserve: new BN(AccountLayout.decode(poolVaultBState.data as any).amount.toString())
