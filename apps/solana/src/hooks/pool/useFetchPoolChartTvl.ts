...
export default function useFetchPoolChartTvl(props: { disable?: boolean; shouldFetch?: boolean; id?: string; refreshInterval?: number }) {
  const { shouldFetch = true, id = '', disable = false, refreshInterval = 1000 * 60 * 5 } = props || {}
  const isValidId = isValidPublicKey(id)
  const [host, lineUrl] = useAppStore((s) => [s.urlConfigs.BASE_HOST, s.urlConfigs.POOL_TVL_LINE], shallow)
